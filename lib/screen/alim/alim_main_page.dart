...
class _AlimScreenState extends State<AlimScreen> {
  ...
  Future<void> restoration() async {
    ...
    try {
      ...
      final response = await http.get(Uri.parse('${config.baseUrl}/allFalse?headDocId=${us.userList[0]['headDocId']}&testCheck=${testcheck}&testMms=${us.userList[0]['mms']}&filterList=${jsonData}'));
      if (response.statusCode == 200) {
