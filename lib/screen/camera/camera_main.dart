...
class _CameraMainState extends State<CameraMain>  {
  ...
  void updateCameraIndex(int oldIndex, int newIndex) async {
    ...
    for (int i = 0; i < _dataL.length; i++) {
      final url = '${config.baseUrl}/updateindex?email=${us.userList[0]['email']}&newindex=${i}&uid=${uidStr[i]}';
  ...
  Future<void> getCameraInit(bool isScroll) async {
    ...
    try {
      ...
      // API 호출로 카메라 정보 가져오기
      final url = '${config.baseUrl}/getNormalCamera?email=${us.userList[0]['email']}&page=${_pageNum}&limit=${_limit}';
  ...
  Future<String> getCameraDetail2(Map<String, dynamic> camera) async {
    ...
    final url = '${config.baseUrl}/getCameraDetail?cameraUid=${camera['cameraUid']}';
    final response = await http.get(Uri.parse(url));
    List<dynamic> data = jsonDecode(response.body);
  ...
  Future<void> cameraDetailInsert(
    ...
    final us = Get.put(UserState());
    final url = '${config.baseUrl}/insertCameraInfo';
