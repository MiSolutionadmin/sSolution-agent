              ...
              onTap: () {
                if (cs.cameraDetailList[0]['currentFirmware'] != cs.cameraDetailList[0]['newFirmware']) {
                  showConfirmTapDialog(context, '새로운 버전이 있습니다 업그레이드 하시겠습니까?', () async {
                    ...
                    bool result = await cs.cameraDevice!.writeCgi('auto_download_file.cgi?server=${config.apiUrl}&file=/updateFirmware&type=0&resevered1=&resevered2=&resevered3=&resevered4=&');
                    if (result) {
            ...
            GestureDetector(
              onTap: () {
                showConfirmTapDialog(context, '해당 기기아이디를 삭제하시겠습니까?', () async {
                  Get.back();
                  final url = '${config.apiUrl}/cameradelete?uid=${cs.cameraUID.value}&email=${us.userList[0]['email']}';
